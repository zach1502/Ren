...
class TempChannels(commands.Cog):
    ...
    async def tempChannelsShow(self, ctx: Context):
        ...
        rolesAllow = [
            discord.utils.get(ctx.guild.roles, id=rid)
            for rid in tempCh[KEY_ROLE_ALLOW]
        ]
        rolesAllow = [roleName.name for roleName in rolesAllow if roleName]
        rolesDeny = [
            discord.utils.get(ctx.guild.roles, id=rid)
            for rid in tempCh[KEY_ROLE_DENY]
        ]
        rolesDeny = [roleName.name for roleName in rolesDeny if roleName]
        categoryName = discord.utils.get(ctx.guild.channels,
        ...
        async with self.config.guild(ctx.guild).all() as guildData:
            if guildData[KEY_ENABLED]:
                ...
                self.logger.info(
                    ...
                    ctx.guild.id)
                await ctx.send(
                    ":negative_squared_cross_mark: TempChannel: Disabled.")
            else:
        ...
        async with self.config.guild(ctx.guild).all() as guildData:
            if guildData[KEY_NSFW]:
                ...
                self.logger.info(
                    ...
                    ctx.author.id)
                await ctx.send(
                    ":negative_squared_cross_mark: TempChannel: NSFW "
    ...
    async def tempChannelsStart(self, ctx: Context, hour: int, minute: int):
        ...
        self.logger.info(
            ...
            ctx.author.name, ctx.author.id, hour, minute, ctx.guild.name,
            ctx.guild.id)
        await ctx.send(
    ...
    async def tempChannelsDuration(self, ctx: Context, hours: int,
                                   minutes: int):
        ...
        self.logger.info(
            ...
            ctx.author.name, ctx.author.id, hours, minutes, ctx.guild.name,
            ctx.guild.id)
    ...
    async def tempChannelsTopic(self, ctx: Context, *, topic: str):
        ...
        self.logger.info(
            ...
            ctx.author.name, ctx.author.id, ctx.guild.name, ctx.guild.id,
            topic)
    ...
    async def tempChannelsName(self, ctx, name: str):
        ...
        self.logger.info("%s (%s) set the channel name to "
                         "%s"
                         " on %s (%s)", ctx.author.name, ctx.author.id, name,
                         ctx.guild.name, ctx.guild.id)

        await ctx.send(
            ":white_check_mark: TempChannel - Name: Channel name set "
            "to: ``{0}``".format(name))
    ...
    async def tempChannelsPosition(self, ctx, position: int):
        ...
        self.logger.info("%s (%s) changed the position to %s on %s (%s)",
                         ctx.author.name, ctx.author.id, position,
                         ctx.guild.name, ctx.guild.id)
    ...
    async def tempChannelsCategory(self,
                                   ctx: Context,
                                   *,
        ...
        if not category:
            ...
            self.logger.info(
                ...
                ctx.author.name, ctx.author.id, category.id, ctx.guild.name,
                ctx.guild.id)
            await ctx.send(":white_check_mark: TempChannel - Category: Parent "
        ...
        async with self.config.guild(ctx.guild).roleAllow() as roleAllow:
            if role.id not in roleAllow:
                ...
                self.logger.info(
                    ...
                    ctx.author.name, ctx.author.id, role.name, ctx.guild.name,
                    ctx.guild.id)
                await ctx.send(
    ...
    @tempChannels.command(name="allowremove",
                          aliases=["allowdelete", "ad", "ar"])
    async def tempChannelsAllowRemove(self, ctx: Context, *,
                                      role: discord.Role):
        ...
        async with self.config.guild(ctx.guild).roleDeny() as roleDeny:
            if role.id not in roleDeny:
                ...
                await ctx.send(
                    ...
                    "than any of the ones in the allowed list.".format(
                        role.name))
            else:
                await ctx.send(
    ...
    @tempChannels.command(name="denyremove",
                          aliases=["denydelete", "dd", "dr"])
    async def tempChannelsDenyRemove(self, ctx: Context, *,
                                     role: discord.Role):
            ...
            if guildData[KEY_CH_CREATED] and guildData[KEY_CH_ID]:
                ...
                try:
                    ...
                    self.logger.info(
                        ...
                        ctx.author.name, ctx.author.id, chanObj.name,
                        chanObj.id, ctx.guild.name, ctx.guild.id)
                    await ctx.send(
                        ":white_check_mark: TempChannel: Channel deleted")
            else:
                await ctx.send(
                    ...
                    try:
                        ...
                        if int(time.strftime("%H")) == guildData[KEY_START_HOUR] and \
                            ...
                            self.logger.info(
                                ...
                                chanObj.name, chanObj.id, guild.name, guild.id)
                            ...
                            if time.time() >= guildData[KEY_STOP_TIME]:
                                self.logger.debug(
                                    "Past channel stop time, clearing ID "
                                    "and created keys.")
